...
from congregation.dag.nodes.internal import Close
from congregation.comp import InsertCloseOps
...
and column-level attributes after the InsertCloseOps() phase
...
@pytest.mark.parametrize("party_data, expected", [
    (
        ...
        {
            "node_order": [Create, Close, Create, Close, Concat, Collect],
            "requires_mpc": [False, True, False, True, True, False],
            "ownership_data":[
                {
                    "stored_with": [{1}],
                    "plaintext_sets": [{1}, {1}],
    ...
    (
        ...
        {
            "node_order": [Create, Create, Concat, Collect],
